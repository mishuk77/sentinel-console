...
import { Play, Loader2, Trophy, ArrowRight, FileText, CheckCircle2, Clock, Cpu, X, AlertTriangle, BarChart2, ShieldAlert } from "lucide-react";
import { cn } from "@/lib/utils";

type TrainingState = 'IDLE' | 'STARTING' | 'TRAINING' | 'COMPLETED';

const FRAUD_TARGET_HINTS = ["is_fraud", "fraud_flag", "fraud", "fraudulent", "label"];
...
export default function FraudTraining() {
    ...
    // Fetch Models — filtered to fraud context on display
    ...
    // Only show fraud models in list
    ...
    useEffect(() => {
        ...
        const currentCount = fraudModels.length;
        const initialCount = initialModelCountRef.current;
        const hasNewModels = currentCount > initialCount;
        const hasTrainingModels = allModels.some(m => m.status === "TRAINING");
    ...
    const trainMutation = useMutation({
        ...
        onSuccess: () => {
            const fraudModels = allModels?.filter(m => (m.metrics as any)?.model_context === "fraud");
            initialModelCountRef.current = fraudModels?.length || 0;
    ...
    // Auto-detect fraud label columns to suggest them
    const suggestedTargets = columns.filter(c =>
        FRAUD_TARGET_HINTS.some(h => c.toLowerCase().includes(h))
    );
    ...
    return (
        <div className="page">
            {/* Header */}
            <div>
                <h1 className="page-title">Fraud Training</h1>
                <p className="page-desc">
                    Train fraud detection models on labeled fraud data.
                </p>
            </div>
            ...
            {isTrainingActive && (
                <div className="panel border-info/30 bg-info/5 animate-in fade-in slide-in-from-top-4">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-info text-sm">
                                    Sentinel is training 3 fraud detection models
                                </p>
                            </div>
                        </div>
                        <button
                    ...
                    {/* Elapsed Time & Status */}
                    <div className="bg-muted/30 rounded-lg p-4 flex items-center justify-between mx-5 mb-5">
                        ...
                        <div className="text-right text-sm text-muted-foreground">
                            {trainingStep === 1 && "Analyzing dataset quality and fraud rate..."}
                            {trainingStep === 2 && "Encoding features, stratified split, SMOTE if needed..."}
                            {trainingStep === 3 && trainingState !== 'COMPLETED' && "Running 5-fold CV · Logistic, Random Forest, XGBoost..."}
                            {trainingStep === 3 && trainingState === 'COMPLETED' && "Finalizing results and building leaderboard..."}
                    ...
                    {trainingState === 'STARTING' && elapsedSeconds >= 45 && (
                        <div className="mx-5 mb-5 p-3 bg-warn/10 border border-warn/30 rounded-lg text-sm text-warn">
            ...
            {isCompleted && (
                <div className="panel border-up/30 bg-up/5 p-5 flex items-center justify-between animate-in zoom-in-95 duration-300">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-lg font-bold">Fraud Models Trained</h3>
                            <p className="text-muted-foreground">
                                {completedAtRef.current != null && completedAtRef.current > 0
                                    ? `Completed in ${formatTime(completedAtRef.current)}. `
                                    : ''}
                                Your fraud detection models are ready for evaluation.
                            </p>
                        </div>
                    </div>
                    <button
                        onClick={() => navigate(`/systems/${systemId}/fraud/models`)}
                        className="btn-primary flex items-center gap-2"
                    >
                        Proceed to Fraud Models <ArrowRight className="h-4 w-4" />
                    </button>
                </div>
            )}
            ...
            {trainingState === 'IDLE' && (
                <div className="panel max-w-3xl">
                    ...
                    {wizardStep === 2 && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                            <div className="flex items-center justify-between border-b pb-4">
                                <h3 className="font-semibold text-lg">3. Configure Fraud Training</h3>
                            </div>
                                ...
                                <div className="grid grid-cols-2 gap-8">
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-sm font-bold flex items-center gap-2">
                                            Fraud Label Column (Y)
                                            <span className="text-xs font-normal text-muted-foreground">(1 = fraud, 0 = legit)</span>
                                        </label>
                                        {suggestedTargets.length > 0 && (
                                            <div className="flex items-center gap-1.5 flex-wrap">
                                                <span className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider">Suggested:</span>
                                                {suggestedTargets.map(col => (
                                                    <button
                                                        ...
                                                        onClick={() => setConfiguration(prev => ({
                                                            ...prev,
                                                            targetCol: col,
                                                            featureCols: prev.featureCols.filter(c => c !== col)
                                                        }))}
                                                        className={cn(
                                                            "text-xs px-2 py-0.5 rounded font-mono border transition-colors",
                                                            configuration.targetCol === col
                                            ...
                                            {columns.map(col => (
                                                <label key={`target-${col}`} className={cn(
                                                    ...
                                                    {suggestedTargets.includes(col) && (
                                                        <span className="ml-auto text-[10px] text-primary font-semibold">fraud label</span>
                                                    )}
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-sm font-bold flex items-center gap-2">
                                            Feature Variables (X)
                                            <span className="text-xs font-normal text-muted-foreground">(Fraud signals)</span>
                                        </label>
                                            ...
                                            {profileData.class_balance != null && (
                                                <div>
                                                    <p className="text-xs font-bold uppercase text-muted-foreground tracking-wider mb-2">
                                                        Fraud Rate — <span className="font-mono text-foreground">{configuration.targetCol}</span>
                                                    </p>
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="text-xs whitespace-nowrap space-x-3 shrink-0">
                                                            <span className="text-down font-medium">Fraud {(profileData.class_balance * 100).toFixed(1)}%</span>
                                                            <span className="text-up font-medium">Legit {((1 - profileData.class_balance) * 100).toFixed(1)}%</span>
                                                        </div>
                                                    </div>
                                                    {profileData.class_balance < 0.01 && (
                                                        <p className="flex items-center gap-1 text-xs text-warn mt-1.5">
                                                            <AlertTriangle className="h-3 w-3" />
                                                            Very low fraud rate — SMOTE oversampling will be applied automatically.
                                                        </p>
                                                    )}
                                                </div>
                                            )}
                            ...
                            <div className="pt-4 border-t space-y-6">
                                ...
                                <div className="flex items-center justify-between">
                                    <p className="text-sm text-muted-foreground">
                                        Sentinel will train multiple fraud models and recommend the best performer based on AUC.
                                    </p>
                                    <button
                                    ...
                                    >
                                        {trainMutation.isPending ? (
                                            ...
                                            <><Play className="mr-2 h-4 w-4" /> Start Fraud Training</>
                                        )}
                                    </button>
                                </div>
                            </div>
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between border-b pb-2">
                    <h3 className="panel-title text-base">Fraud Model Results</h3>
                </div>
                ...
                {models && models.length > 0 ? (
                    ...
                    !isTrainingActive && (
                        <div className="p-12 text-center bg-muted/20 rounded-xl border border-dashed">
                            ...
                            <h3 className="text-base font-semibold text-foreground mb-2">No Fraud Models Yet</h3>
                            <p className="text-muted-foreground text-sm max-w-md mx-auto mb-4">
                                {datasets && datasets.length > 0
                                    ? "Use the wizard above to configure and start your first fraud training run."
                                    : "Upload a fraud dataset first, then return here to train models."}
                            </p>
                            {(!datasets || datasets.length === 0) && (
                                <button
                                    onClick={() => navigate(`/systems/${systemId}/fraud/data`)}
                                    className="inline-flex items-center gap-2 text-sm font-medium text-primary hover:underline"
                                >
                                    <FileText className="h-4 w-4" />
                                    Upload Fraud Dataset First
                                </button>
                            )}
                        </div>
                    )
